...
func TestRechargeEpayAllowsWalletBalanceAboveBillingClamp(t *testing.T) {
	oldQuotaPerUnit := common.QuotaPerUnit
	common.QuotaPerUnit = 500000
	t.Cleanup(func() { common.QuotaPerUnit = oldQuotaPerUnit })
	...
	testCases := []struct {
		...
		{
			name:         "crosses billing clamp",
			currentQuota: common.MaxQuota - 1 - 1_000_000,
			wantQuota:    common.MaxQuota - 1,
		},
		{
			name:         "adds to wallet already above billing clamp",
			currentQuota: common.MaxQuota + 10_000_000,
			wantQuota:    common.MaxQuota + 11_000_000,
		},
	}
