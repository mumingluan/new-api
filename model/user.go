...
// DeductUserQuotaAllowArrears applies a settlement adjustment that may make
// the wallet balance negative. Normal pre-consume paths must use
// DecreaseUserQuota or TryReserveUserQuota instead.
func DeductUserQuotaAllowArrears(id int, quota int) error {
	if quota < 0 {
		return errors.New("quota 不能为负数！")
	}
	if quota == 0 {
	...
	gopool.Go(func() {
		if err := cacheDecrUserQuota(id, int64(quota)); err != nil {
			common.SysLog("failed to decrease user quota for arrears: " + err.Error())
		}
	})
	return nil
}
