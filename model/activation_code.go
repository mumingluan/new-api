...
const (
	...
	legacyActivationCodeGroup    = "普通用户"
)
...
var (
	...
	ErrActivationGroupMismatch     = errors.New("激活码分组与密钥分组不一致")
...
func validateUsableActivationCode(code *ActivationCode, now int64) error {
	...
	if strings.TrimSpace(code.Group) == "" {
		return fmt.Errorf("%w: 激活码分组为空", ErrActivationRequestInvalid)
	}
	return nil
}

func migrateActivationCodeGroups() error {
	return DB.Model(&ActivationCode{}).
		Where(commonGroupCol+" IS NULL OR "+commonGroupCol+" = ?", "").
		Update("group", legacyActivationCodeGroup).Error
}
...
func GetActivationPrecheck(code, qq, apiKey string, now int64) (*ActivationPrecheck, error) {
	...
	if token.Group != activationCode.Group {
		return nil, ErrActivationGroupMismatch
	}
...
func RenewActivationCode(code, apiKey, clientIp string, now int64) (*ActivationResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if token.Group != activationCode.Group {
			return ErrActivationGroupMismatch
		}
		claimed := tx.Model(&ActivationCode{}).
			Where("id = ? AND status = ?", activationCode.Id, ActivationCodeStatusActive).
			Updates(map[string]any{"status": ActivationCodeStatusUsed, "used_time": now})
...
func CreateUserActivationCodes(userId, count, days int, channel, group string, expiredTime int64, suffixes []string) ([]ActivationCode, error) {
